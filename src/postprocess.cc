...
extern "C" {
  ...
  static int construct_tree (gptl_private::Timer *timerst);
  static char *methodstr (GPTL_Method method);
  static int newchild (gptl_private::Timer *parent, gptl_private::Timer *child);
  static int get_outputfmt (const gptl_private::Timer *ptr, const int depth, const int indent, Outputfmt *outputfmt);
  static void fill_output (int depth, int namelen, int chars2pr, Outputfmt *outputfmt);
  static int get_max_namelen (gptl_private::Timer *timers);
  static int is_descendant (const gptl_private::Timer *node1, const gptl_private::Timer *node2);
  static int is_onlist (const gptl_private::Timer *child, const gptl_private::Timer *parent);
  static void printstats (const gptl_private::Timer *timer, FILE *fp, int t, int depth, bool doindent,
			  double self_ohd, double parent_ohd, const Outputfmt outputfmt);
  static void print_multparentinfo (FILE *fp, gptl_private::Timer *ptr);
  static void add (gptl_private::Timer *tout, const gptl_private::Timer *tin);
  static void printself_andchildren (const gptl_private::Timer *ptr, FILE *fp, int t, int depth, 
				     double self_ohd, double parent_ohd, Outputfmt outputfmt);
  static void print_hashstats (FILE *fp);
  static float meanhashvalue (gptl_private::Hashentry *hashtable, int tablesize);
  static void print_memstats (FILE *fp, gptl_private::Timer **timers, int tablesize, int maxthreads);
...
int GPTLpr_file (const char *outfile)
{
  ...
  using gptl_thread::nthreads;
  using gptl_postprocess::method;
  FILE *fp;                 // file handle to write to
  gptl_private::Timer *ptr;               // walk through master thread linked list
  gptl_private::Timer *tptr;              // walk through slave threads linked lists
  gptl_private::Timer *sumstats;          // sum of same timer stats over threads
  ...
  if (funclist[funcidx].option == GPTLnanotime) {
    ...
    fprintf (fp, "Source of clock rate was %s\n", gptl_once::clock_source);
    if (strcmp (gptl_once::clock_source, "/proc/cpuinfo") == 0) {
  ...
  for (t = 0; t < nthreads; ++t) {
    ...
    */
	...
	printstats (ptr, fp, t, 0, false, self_ohd, parent_ohd, outputfmt);
      ...
      printself_andchildren (gptl_private::timers[t], fp, t, -1, self_ohd, parent_ohd, outputfmt);
    }
    ...
    if (gptl_private::wallstats.enabled && gptl_private::overheadstats.enabled)
      fprintf (fp, "\n");
  ...
  if (gptl_once::dopr_threadsort && nthreads > 1) {
    int nblankchars;
    ...
    if (gptl_private::cpustats.enabled)
      fprintf (fp, "%9s", gptl_private::cpustats.str);
    if (gptl_private::wallstats.enabled) {
      fprintf (fp, "%9s", gptl_private::wallstats.str);
      if (gptl_once::percent && gptl_private::timers[0]->next)
        fprintf (fp, " %%_of_%4.4s ", gptl_private::timers[0]->next->name);
      if (gptl_private::overheadstats.enabled)
        fprintf (fp, "%9s", gptl_private::overheadstats.str);
    }
#ifdef HAVE_PAPI
    ...
    sumstats = new gptl_private::Timer ("none", NULL);
    for (ptr = gptl_private::timers[0]->next; ptr; ptr = ptr->next) {      
      ...
      *sumstats = *ptr;
      for (t = 1; t < nthreads; ++t) {
        found = false;
        for (tptr = gptl_private::timers[t]->next; tptr && ! found; tptr = tptr->next) {
  ...
  if (gptl_once::dopr_multparent && ! gptl_private::imperfect_nest) {
    for (t = 0; t < nthreads; ++t) {
      ...
      if (some_multparents) {
        ...
        if (gptl_once::dopr_preamble && t == 0) {
  ...
  if (gptl_once::dopr_collision)
    print_hashstats (fp);
  // Print stats on GPTL memory usage
  print_memstats (fp, gptl_private::timers, gptl_private::tablesize, gptl_thread::maxthreads);
...
extern "C" {
  ...
  */
  static void print_titles (int t, FILE *fp, Outputfmt *outputfmt)
  {
    ...
    */
    ...
    if (gptl_private::imperfect_nest) {
      outputfmt->max_namelen  = get_max_namelen (gptl_private::timers[t]);
      outputfmt->max_chars2pr = outputfmt->max_namelen;
      nblankchars             = outputfmt->max_namelen + 1;
    } else {
      if (construct_tree (gptl_private::timers[t]) != 0)
	printf ("GPTL: %s: failure from construct_tree: output will be incomplete\n", thisfunc);
      ...
      ret = get_outputfmt (gptl_private::timers[t], -1, indent_chars, outputfmt);
#ifdef DEBUG
  ...
  */
  static int construct_tree (gptl_private::Timer *timerst)
  {
    gptl_private::Timer *ptr;       // traverse linked list
    gptl_private::Timer *pptr = 0;  // parent (init to NULL to avoid compiler warning)
    ...
    for (ptr = timerst; ptr; ptr = ptr->next) {
      switch (gptl_postprocess::method) {
	...
	return gptl_util::error ("GPTL: %s: method %d is not known\n",
				 thisfunc, gptl_postprocess::method);
      }
    }
    return 0;
  }
  ...
  */
  static int newchild (gptl_private::Timer *parent, gptl_private::Timer *child)
  {
    ...
    gptl_private::Timer **chptr;     // array of pointers to children
    ...
    if ( ! is_onlist (child, parent)) {
      ...
      chptr = (gptl_private::Timer **) realloc (parent->children, nchildren * sizeof (gptl_private::Timer *));
      if ( ! chptr)
  ...
  */
		   ...
		   double self_ohd, double parent_ohd, const Outputfmt outputfmt)
  {
	...
	fprintf (fp, " %8.3f", wallmin);
      ...
      if (gptl_once::percent && gptl_private::timers[0]->next) {
	ratio = 0.;
	if (gptl_private::timers[0]->next->wall.accum > 0.)
	  ratio = (timer->wall.accum * 100.) / gptl_private::timers[0]->next->wall.accum;
  ...
  static void print_memstats (FILE *fp, gptl_private::Timer **timers, int tablesize, int maxthreads)
  {
    ...
    // fixed size of table
    hashmem = (float) sizeof (gptl_private::Hashentry) * tablesize * maxthreads;
    callstackmem = (float) sizeof (gptl_private::Timer *) * MAX_STACK * maxthreads;
    for (int t = 0; t < gptl_thread::nthreads; t++) {
      numtimers = 0;
      for (ptr = timers[t]->next; ptr; ptr = ptr->next) {
	++numtimers;
	pchmem  += (float) sizeof (gptl_private::Timer *) * (ptr->nchildren + ptr->nparent);
      }
      hashmem   += (float) numtimers * sizeof (gptl_private::Timer *);
      regionmem += (float) numtimers * sizeof (gptl_private::Timer);
#ifdef HAVE_PAPI
  ...
  */
  static int rename_duplicate_addresses ()
  {
    gptl_private::Timer *ptr;       // iterate through timers
    gptl_private::Timer *testptr;   // start at ptr, iterate through remaining timers
