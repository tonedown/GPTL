...
namespace gptl_papi {
  int npapievents = 0;              // number of PAPI events: initialize to 0 
  int nevents = 0;                  // number of events: initialize to 0 
  int *EventSet;                    // list of events to be counted by PAPI
  ...
  extern "C" {
    /*
    ** PAPIsetoption: enable or disable PAPI event defined by "counter". Called 
    ...
    */
    ...
    int PAPIsetoption (const int counter, const int val)
    {
      ...
      char eventname[PAPI_MAX_STR_LEN]; // returned from PAPI_event_code_to_name
      static const char *thisfunc = "PAPIsetoption";
    ...
    /*
    ** PAPIinitialize(): Initialize the PAPI interface. Called from gptl_once::initialize.
    ...
    **
    ** Return value: 0 (success) or gptl_util::error or -1 (failure)
    */
	...
	return gptl_util::error ("%s: PAPI_thread_init failure\n", thisfunc);
      ...
      EventSet     = (int *)        allocate (maxthreads * sizeof (int), thisfunc);
      papicounters = (long_long **) allocate (maxthreads * sizeof (long_long *), thisfunc);

      for (t = 0; t < maxthreads; t++) {
	EventSet[t] = PAPI_NULL;
	papicounters[t] = (long_long *) allocate (MAX_AUX * sizeof (long_long), thisfunc);
      }
    ...
    /*
    ** create_and_start_events: Create and start the PAPI eventset.
    ...
    */
    ...
    /*
    ** PAPIstart: Start the PAPI counters (actually they are just read).  
    **   Called from GPTLstart* routines.
    **
    ** Input args:  
    **   t: thread number
    **
    ...
    */
    ...
    /*
    ** PAPIstop: Stop the PAPI counters (actually they are just read).  
    ...
    */
    ...
    /*
    ** PAPIprstr: Print the descriptive string for all enabled PAPI events.
    **   Called from GPTLpr.
    **
    ** Input args: 
    **   fp: file descriptor
    */
    void PAPIprstr (FILE *fp)
    {
      for (int n = 0; n < nevents; n++) {
    ...
    */
    ...
    /*
    ** PAPIadd: Accumulate PAPI counters. Called from add.
    **
    ** Input/Output args: 
    ...
    */
    ...
    /*
    ** PAPIquery: return current PAPI counter info. Return into a long for best
    ...
    */
    ...
    /*
    ** PAPIget_eventvalue: return current value for an enabled event.
    **
    ** Input args:
    ...
    */
	...
	if (STRMATCH (eventname, pr_event[n].event.namestr)) {
      ...
      if (n == nevents)
	return gptl_util::error ("%s: event %s not enabled\n", thisfunc, eventname);
      return 0;
    }
