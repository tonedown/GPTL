...
// Use static functions and data for file-local access
// __cyg functions need to be publicly available due to -finstrument-functions flag
...
extern "C" {
  void __cyg_profile_func_enter (void *this_fn, void *call_site)
  {
    ...
    ptr = gptl_autoinst::getentry_instr (t, this_fn, &indx);
...
// Functions needed by other GPTL routines and files
...
#undef APPEND_ADDRESS
    ...
    */
    inline gptl_private::Timer *getentry_instr (const int t, void *self, unsigned int *indx)
    {
      ...
      *indx = (((unsigned long) self) >> 4) % gptl_private::tablesize;
      for (int i = 0; i < gptl_private::hashtable[t][*indx].nument; ++i) {
	if (gptl_private::hashtable[t][*indx].entries[i]->address == self) {
	  ptr = gptl_private::hashtable[t][*indx].entries[i];
	  break;
	}
      }
      return ptr;
...
static void __func_trace_enter (const char *function_name, const char *file_name, int line_number,
